...
void correct()
{
	...
	while (times--)
	{
		...
		d1 = (double)end / CLOCKS_PER_SEC * 1000. / 1000;  // 所花时间（毫秒）
		printf("time to generate a public key is: %f ms\n\n", d1);
		...
		d1 = (double)end / CLOCKS_PER_SEC * 1000. / 1000;  // 所花时间（毫秒）
		printf("time to do a sign is: %f ms\n\n", d1);
		...
		d1 = (double)end / CLOCKS_PER_SEC * 1000. / 1000;  // 所花时间（毫秒）
		printf("time to do a verify is: %f ms\n\n", d1);
	}// 循环一次大概做4次点乘
