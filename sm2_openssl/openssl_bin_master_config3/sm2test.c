...
// 	size_t inlen = 6;
	...
	if (!BN_hex2bn(&a, "FFFFFFFEFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF00000000FFFFFFFFFFFFFFFC")) ABORT;
	if (!BN_hex2bn(&b, "28E9FA9E9D9F5E344D5A9E4BCF6509A7F39789F515AB8F92DDBCBD414D940E93")) ABORT;
	...
	if (!BN_hex2bn(&x, "32C4AE2C1F1981195F9904466A39C9948FE30BBFF2660BE1715A4589334C74C7")) ABORT;
	if (!EC_POINT_set_compressed_coordinates_GFp(group, P, x, 0, ctx)) ABORT;
	if (!EC_POINT_is_on_curve(group, P, ctx)) ABORT;
	if (!BN_hex2bn(&z, "FFFFFFFEFFFFFFFFFFFFFFFFFFFFFFFF7203DF6B21C6052B53BBF40939D54123")) ABORT;
...
void report_time( struct timeval start, struct timeval end, int times) {
	double d1;
...
// 	size_t inlen = 6;
	...
	//if (!EC_POINT_mul(group, R, scalar[0], NULL, NULL, ctx)) ABORT;

	fprintf(stdout, "\nPoint mul without pre-compute ...\n");
	for (i=0; i<10000; i++) {
		scalar[i] = BN_new();
		if (!BN_rand_range(scalar[i], order)) ABORT;
	}
	gettimeofday(&start, NULL);
	for (i=0; i<10000; i++) {
		if (!EC_POINT_mul(group, Q, scalar[i], NULL, NULL, ctx)) ABORT;
		if (!EC_POINT_get_affine_coordinates_GFp(group, P, x, y, ctx)) ABORT;
	}
	gettimeofday(&end, NULL);
	report_time(start, end, 10000);

#if 0
	fprintf(stdout, "\nPoint mul-add without pre-compute ...\n");
	gettimeofday(&start, NULL);
	for (i=0; i<1000; i++) {
		if (!EC_POINT_mul(group, Q, scalar[i], R, scalar[999-i], ctx)) ABORT;
	}
	gettimeofday(&end, NULL);
	report_time(start, end, 1000);

	fprintf(stdout, "\nPoint mul with pre-compute ...\n");
	for (i=0; i<1000; i++) {
		scalar[i] = BN_new();
		if (!BN_rand_range(scalar[i], order)) ABORT;
	}
	...
	fprintf(stdout, "\nPoint mul-add with pre-compute ...\n");
	gettimeofday(&start, NULL);
	for (i=0; i<1000; i++) {
		if (!EC_POINT_mul(group, Q, scalar[i], R, scalar[999-i], ctx)) ABORT;
	}
